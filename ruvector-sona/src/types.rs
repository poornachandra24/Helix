...
impl LearningSignal {
    ...
    fn estimate_gradient(trajectory: &QueryTrajectory) -> Vec<f32> {
        ...
        let baseline = if trajectory.steps.len() <= 1 {
            0.0
        } else {
            trajectory.steps.iter().map(|s| s.reward).sum::<f32>() / trajectory.steps.len() as f32
        };

        // REINFORCE: gradient = sum((reward - baseline) * (target - query)) if target exists
        for step in &trajectory.steps {
            let advantage = step.reward - baseline;
            if step.attention_weights.len() == dim && step.activations.len() == dim {
                for i in 0..dim {
                    let diff = step.attention_weights[i] - step.activations[i];
                    gradient[i] += advantage * diff;
                }
            } else {
