...
impl SonaEngine {
    ...
    /// Apply micro-LoRA and sequential base-LoRA layers to hidden states
    pub fn apply_micro_lora(&self, input: &[f32], output: &mut [f32]) {
        ...
        // 2. Apply BaseLoRA layers sequentially to consolidate background learning
        if let Some(lora) = self.coordinator.base_lora().try_read() {
            let mut temp_input = input.to_vec();
            for (ti, &out_val) in temp_input.iter_mut().zip(output.iter()) {
                *ti += out_val;
            }

            let mut temp_output = vec![0.0f32; input.len()];
            for layer_idx in 0..lora.num_layers() {
                temp_output.fill(0.0);
                lora.forward_layer(layer_idx, &temp_input, &mut temp_output);
                for (ti, &to_val) in temp_input.iter_mut().zip(temp_output.iter()) {
                    *ti += to_val;
                }
                for (o, &to_val) in output.iter_mut().zip(temp_output.iter()) {
                    *o += to_val;
                }
            }
        }
