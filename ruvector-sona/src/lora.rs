...
impl MicroLoRA {
    ...
    pub fn accumulate_gradient(&mut self, signal: &LearningSignal) {
        if signal.gradient_estimate.len() != self.hidden_dim
            || signal.query_embedding.len() != self.hidden_dim
        {
            return;
        }

        let quality = signal.quality_score;
        let input = &signal.query_embedding;
        let grad_out = &signal.gradient_estimate;

        // 1. Compute intermediate activations: intermediate = input @ down
        let mut intermediate = vec![0.0f32; self.rank];
        for r in 0..self.rank {
            let mut sum = 0.0f32;
            let offset = r * self.hidden_dim;
            for i in 0..self.hidden_dim {
                if offset + i < self.down_proj.len() {
        ...
        // 2. Accumulate gradient for up_proj: d(loss)/d(W_up) = grad_out * intermediate * scale
        for r in 0..self.rank {
            for i in 0..self.hidden_dim {
                let grad_idx = r * self.hidden_dim + i;
                if grad_idx < self.grad_up.len() {
                    self.grad_up[grad_idx] += grad_out[i] * intermediate[r] * self.scale * quality;
                }
            }
        }
    ...
    pub fn new(hidden_dim: usize, rank: usize, num_layers: usize) -> Self {
        let layers = (0..num_layers)
            .map(|idx| {
                let down_proj: Vec<f32> = (0..hidden_dim * rank)
                    .map(|i| {
                        let x = ((i + idx * hidden_dim * rank) as f32 * 0.618_034) % 1.0;
                        (x - 0.5) * 0.02
                    })
                    .collect();
                LoRALayer {
